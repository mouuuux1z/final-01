...
import type { ApiResponse, ClinicProfile, PaginatedResponse } from '../../types';
import type { AdminStackParamList } from '../../navigation/AdminStack';
import { UI } from '../../theme/ui';

type Props = NativeStackScreenProps<AdminStackParamList, 'PendingClinics'>;
...
export function AdminPendingClinicsScreen({ navigation }: Props) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();

  const { data, isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['admin', 'pending-clinics'],
    queryFn: async () => {
      const { data: response } = await api.get<ApiResponse<PaginatedResponse<ClinicProfile>>>(
  ...
  const verifyMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: 'ACTIVE' | 'SUSPENDED' }) =>
      api.patch(`/admin/clinics/${id}`, { status }),
    onSuccess: () => {
  ...
  return (
    <View className="flex-1 pt-14">
      <View className="mb-4 px-6">
        ...
        <Text className="text-2xl font-bold text-slate-900">{t('admin.pendingClinics')}</Text>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="mt-16 items-center">
              ...
              <Text className="mt-4 text-slate-500">{t('admin.noPendingClinics')}</Text>
            </View>
          }
          renderItem={({ item }) => {
              ...
              <View className="mb-4 rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
                <View className="mb-3 flex-row items-start">
                  <View className="mr-3 h-12 w-12 items-center justify-center rounded-card bg-primary-light">
                    <AppIcon name="clinic" size={22} color={UI.primary} />
                  </View>

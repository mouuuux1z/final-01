...
export function ClinicDoctorsScreen({ navigation }: Props) {
  ...
  const { data: clinic, isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['clinic', 'me'],
    queryFn: async () => {
  ...
  return (
    <View className="flex-1 pt-14">
      <View className="mb-4 px-6">
        <Text className="text-2xl font-bold text-slate-900">{t('clinic.manageDoctors')}</Text>
        <Text className="mt-1 text-sm text-slate-500">{t('clinic.manageDoctorsHint')}</Text>
      </View>
      <View className="mb-4 px-6">
